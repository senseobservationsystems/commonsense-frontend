...
class Ckan_client
{
	 ...
	 * @since	Version 0.1.0
	 */
	private $base_url = 'http://data.rotterdamopendata.nl:9090/nl/api/%d/';
	...
	public function put_group_entity($group, $data)
	{	
		return $this->make_request('PUT', 
	...
	public function search_package($keywords, $opts = array())
	{
		...
		return $data = $this->make_request('GET', 
			$this->resources['package_search'] . '?name=' .
			urlencode($keywords));
	...
	private function make_request($method, $url, $data = FALSE)
	{
		...
		if ($method === 'POST' || $method === 'PUT')
		{
			...
			else
			{
				...
				if($method == 'PUT'){
					curl_setopt($this->ch, CURLOPT_PUT, $data);	
				}
		...
		{
			//throw new Exception($info['http_code'] . ': ' . $this->http_status_codes[$info['http_code']]);
			return $info['http_code'];
		}
