...
public class SensorDeleteDialog extends View {
	...
	private void onRemoveFailure() {
		text.setHtml("Removal failed, retry?");
		setBusy(false);
		window.show();
	}
	...
	private void onShow(final List<ExtSensor> sensors) {
		...
		String message = "<p>Are you sure you want to remove the selected sensor from your list?</p>";
		if (sensors.size() > 1) {
			message = "<p>Are you sure you want to remove all " + sensors.size()
					+ " selected sensors from your list?</p>";
		}
		message += "<br/><p>Warning: the removal can not be undone! Any data you stored for this sensor will be lost. Forever.</p>";
