...
public class TimeLinePanel extends VizPanel {
	...
	private HashMap<String, HashMap<String, String>> groupLabels = new HashMap<String, HashMap<String, String>>();
	...
	private DataTable createDataTable(JsArray<Timeseries> stringData) {
		...
		for (int i = 0; i < stringData.length(); i++) {
			...
			for (int j = 0, index = dataTable.getNumberOfRows(); j < values.length(); j++) {
				...
				String cls = getLabelClass(ts.getLabel(), dataPoint.getRawValue());
				...
				if (j > 0) {
					if (false == (lastPoint != null && lastPoint.getRawValue().equals(
						...
						dataTable.setValue(index, 2,
								"<div class=\"" + cls + "\">" + dataPoint.getRawValue() + "</div>");
	 ...
	 * @param group
	 * @param label
	 * @return A CSS class for divs with timeline values
	 */
	private String getLabelClass(String group, String label) {

		HashMap<String, String> labels = groupLabels.get(group);
		if (null == labels) {
			labels = new HashMap<String, String>();
		}
		String labelClass = labels.get(label);
		if (labelClass == null) {
			labelClass = "timeline-event-label timeline-event-label-" + (labels.size() % 16);
			labels.put(label, labelClass);
			groupLabels.put(group, labels);
		}
