package nl.sense_os.commonsense.main.client.viz.data.cache;
...
final class CacheJso extends JavaScriptObject {
     ...
     *            JavaScript does not have long type.
     */
    protected native JsArray<Timeseries> request(JsArrayString ids, double start, double end) /*-{
		var result = [];
			...
			for ( var j = 0; j < this.content.length; j++) {
				...
				if (timeseries.id == id) {
					...
					for ( var k = 0; k < timeseries.data.length; k++) {
						...
						if (dataPoint.date >= start
								&& (end == -1 || dataPoint.date <= end)) {
