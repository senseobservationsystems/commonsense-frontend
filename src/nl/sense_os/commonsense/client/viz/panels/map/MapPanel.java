...
//	     	   	LOG.fine ("Setting Min value end slider is "+ calculDate(endSlider.getMinValue()*1000l));
	...
	private void drawIcon() {
		...
		for (int j = 0; j < bigList.size(); j++) {	
	        ...
	        //LOG.fine ("Drawing live marker at " + firstPoint.getLatitude() + " " + firstPoint.getLongitude());
	...
	/** Gets the timer interval based on the ANIMATION_DURATION and the distance between adjacent points
	 * 
	 * @param bigListIndex
	 * @param index
	 * @return
	...
	private void updateIconIndex() {
		...
		for (int i = 0; i < bigList.size(); i++ ) {
			...
			for (int j = 1; j <= timestamps.size(); j++) {
				...
				if (j < timestamps.size()) trialTimestamp = timestamps.get(j);
				else trialTimestamp = timestamps.get(j-1);
				if (j < timestamps.size() && trialTimestamp > timestamp) {
				
					indexList.set(i, (j-1));
					//LOG.fine ("j is " + j + "Points size is "+ points.size() + "TrialTimestamp has index " + j);
...
//        					" The length of lonPoints  for " + Id_names.get(r) + " is " + lonPoints.size());     	
	...
	/** calculate distance and speed between adjacent points of the given ID, and select out only good points
	    ...
	    private int filterPoints(ArrayList<Float> latPoints, ArrayList<Float> lonPoints, ArrayList <Long> timestamps, int r, int goodStart) {
	    	...
	    	// create an arrayList to store indices of "bad points" that have to be discarded for this ID
	...
	//						" its newLatit is " + newLatit + " newLongit is " + newLongit);
					...
					if (distance > 0) IdDistanceRange = IdDistanceRange + distance;
			...
			if (IdGoodLat.size()> IdBadPoints.size()) {
				...
				IdTimeRange = IdGoodTimestamps.get(IdGoodTimestamps.size() - 1) - IdGoodTimestamps.get(0);
	...
	/** Selects only the points between which the distance is significant compared to the total length of the trace
	 * 
	 * @param bigListIndex
	 */
...
//	
	...
	private void add500AnimatedPoints() {
		...
		for (int i = 0; i < bigList.size(); i++ ) {
			...
			for (int j = 1; j < animatePoints.size(); j++ ) {
				...
				if (distance < distanceRange500) {
					...
					while (done == false) {	
						...
						double addLatit = latit + k * (newLatit - latit)* (distanceRange500/ distance);
						double addLongit = longit + k * (newLongit - longit) * (distanceRange500/ distance);
						long addTimestamp = timestamp + (long) (k * (newTimestamp - timestamp) * (distanceRange500/ distance));
						...
						if (distancePast < distance) {
							...
							LOG.fine ("Distance " + j + " is " + distance + " Done with point " + j + " added " + k + " points ");
						}
					}				
				}
			...
			" the length of allAnimatedTimeProportions for " + i + " is " + animatedTimeProportions.size());
...
//               LOG.fine("traceStartIndex for " + " is " + traceStartIndex + " traceEndIndex is " + traceEndIndex);
                ...
                if (ANIMATE == false) map.addOverlay(startMarker);              
                
                // Add the last marker
                endMarker = new Marker(points[lastPoint], markerOptions);
                endMarkerList.add(endMarker);
                if (ANIMATE == false) map.addOverlay(endMarker);
                ...
                polyList.add(trace);
                if (ANIMATE == false) map.addOverlay(trace);
