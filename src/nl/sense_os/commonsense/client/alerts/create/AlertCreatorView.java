...
public class AlertCreatorView extends View {
    ...
    private static final Logger LOG = Logger.getLogger(AlertCreatorView.class
	    .getName());
	...
	default:
	    ...
	    MessageBox
		    .alert("Alert creator",
			    "Cannot create alerts for this sensor: incompatible data type.",
			    null);
	    return;
	}
    }
	...
	creator.getNextButton().addSelectionListener(
	...
	creator.getBackButton().addSelectionListener(
		new SelectionListener<ButtonEvent>() {
	...
	creator.getCancelButton().addSelectionListener(
		new SelectionListener<ButtonEvent>() {
	...
	if (-1 != timestamp) {
	    ...
	    DataRequestEvent event = new DataRequestEvent(start, end, sensors,
		    subsample, showProgress);
