...
public class MapPanel extends ContentPanel {
    ...
    /**
     * Create a slider on the bottom, to filter the points to draw according to a time specified
     * with the slider.
     */
    ...
    /**
     * This method is called when it used the slider below the map. It filters the points to draw a
     * line depending on the time.
     * 
     * @param time
     */
    ...
    public void addData(TreeModel sensor, SensorValueModel[] data) {
        ...
        if (data.length > 0) {
            ...
            /*
             * LatLngBounds bounds = map.getBounds(); bounds.extend(startMarker.getLatLng());
             * bounds.extend(endMarker.getLatLng());
             * 
             * int zoomLevel = map.getBoundsZoomLevel(bounds); Log.d(TAG, "zoom level: " +
             * zoomLevel);
             */

            // Adjust the zoom level according to the bounds.
            /*
             * if (zoomLevel < 9) map.setZoomLevel(zoomLevel); else map.setZoomLevel(12);
             * 
